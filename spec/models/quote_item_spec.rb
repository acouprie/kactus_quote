...
RSpec.describe QuoteItem, type: :model do
  ...
  it { is_expected.to validate_inclusion_of(:vat_rate).in_array([ 0, 5.5, 10, 20 ]) }

  describe "quantity" do
    it {
      is_expected.to validate_numericality_of(:quantity)
        .is_greater_than(0)
        .is_less_than_or_equal_to(99_999.99)
    }
    ...
    it "reads a French decimal separator, normalized before assignment" do
      ...
      expect(item.quantity).to eq(12.50)
    end
  ...
  describe "unit_price_excl_vat" do
    ...
    it "reads a French decimal separator, normalized before assignment" do
      ...
      expect(item.unit_price_excl_vat).to eq(12.50)
    end
